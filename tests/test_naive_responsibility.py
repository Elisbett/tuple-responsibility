...
def backend() -> SQLiteBackend:
    """Open the smoke-test database with disabled columns set up."""
...
def test_naive_computes_expected_scores(backend: SQLiteBackend) -> None:
...
def test_naive_min_contingency_sizes(backend: SQLiteBackend) -> None:
...
def test_ranking_sorted_by_responsibility(backend: SQLiteBackend) -> None:
